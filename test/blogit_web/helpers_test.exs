defmodule BlogitWeb.HelpersTest do
  ...
  def with_phoexni_endpoint(conn) do
    private = Map.put(conn.private, :phoenix_endpoint, BlogitWeb.Endpoint)
    Map.put(conn, :private, private)
  end

  describe "localized_path" do
    test "generates the same path as `BlogitWeb.Router.Helpers.post_path` " <>
    "if calling `BlogitWeb.Helpers.current_locale()` returns " <>
    "the default locale", %{conn: conn} do
      Gettext.put_locale(BlogitWeb.Gettext, Blogit.Settings.default_language())

      path = BlogitWeb.Helpers.localized_path(conn, "post", :index)

      assert path == BlogitWeb.Router.Helpers.post_path(conn, :index)
    end
    ...
    "the default locale", %{conn: conn} do
      ...
      Gettext.put_locale(BlogitWeb.Gettext, "pl")

      path = BlogitWeb.Helpers.localized_path(conn, "post", :index)

      assert path == "/pl#{BlogitWeb.Router.Helpers.post_path(conn, :index)}"
    end
  end
